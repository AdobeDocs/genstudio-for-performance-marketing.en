...
| **[!UICONTROL CPA]**<br>_Cost per Action_ | Average cost ($) for a specific action taken by a user, such as a purchase or subscription.<br>**Calculation**: total amount `spent` divided by `clicks` (the number of actions completed), then multiplied by 1000 | Use to monitor spending on experiences that result in valuable actions. |
