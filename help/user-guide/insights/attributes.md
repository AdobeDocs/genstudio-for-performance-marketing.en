...
| **[!UICONTROL CPA]**<br>_Cost per Action_ | Average cost ($) for a specific customer action, such as a purchase or subscription.<br>**Calculation**: total amount `spent` divided by the number of actions completed | Helps to identify attributes that result in valuable actions. |
